...
export async function generateDigest(input: {
  ...
  const system = [
    ...
    `{"headline":string,"summary":string,"highlights":[{"actor":string|null,"action":string,"task":string|null}]}`,
    "Rules:",
...
/** Merge the model draft with deterministic data into a stored digest body. */
export function toDigestContent(
  draft: DigestDraft,
  stats: DigestStats,
  people: DigestPerson[],
): DigestContent {
  return { ...draft, stats, people };
}
